...
static void HeapFragmentation_ReadSizes() {
    ...
    u32 zMaxFree, zFree, zAlloc;
    ArenaImpl_GetSizes(&sHeapFragmentationZeldaArena, &zMaxFree, &zFree, &zAlloc);
}
...
static void HeapFragmentation_SystemAllocR(uintptr_t ptr, size_t size) {
    sHeapFragmentationSystemArenaMap[ptr] = (uintptr_t)__osMallocR(&sHeapFragmentationSystemArena, size);
}

static void HeapFragmentation_SystemFree(uintptr_t ptr) {
    if (sHeapFragmentationSystemArenaMap[ptr] != (uintptr_t) nullptr) {
...
static void HeapFragmentation_ZFree(uintptr_t ptr) {
    if (sHeapFragmentationZeldaArenaMap[ptr] != (uintptr_t) nullptr) {
        __osFree(&sHeapFragmentationZeldaArena, (void*)sHeapFragmentationZeldaArenaMap[ptr]);
    }
    sHeapFragmentationZeldaArenaMap.erase(ptr);
}
...
void RegisterHeapFragmentation() {
    ...
    COND_VB_SHOULD(VB_LOAD_ACTOR, CVAR_HEAP_FRAGMENTATION_NAME, {
        ...
        if (!sHeapFragmentationZeldaArenaMap.contains(actorAddr)) {
            // Error! This should be always be true
            return;
        }

        if (sHeapFragmentationZeldaArenaMap.at(actorAddr) == (uintptr_t) nullptr) {
            *should = false;
        }
    });
}
