...
void BuildEnHyMessage_BlueFire(IEvent* event) {
    ...
    CustomMessage msg = CustomMessage(
        ...
        "%cFeu bleu%w ! Je l'�change contre %rquelque chose de sp�cial%w. Pas de retour ! Je re�ois de nouveaux "
        "articles sp�ciaux %btous les 7 ans environ%w...");
    msg.AutoFormat();
    msg.LoadIntoFont();
...
void BuildEnHyMessage_Fish(IEvent* event) {
    ...
    CustomMessage msg = CustomMessage(
        ...
        "besondere Ware %balle 7 Jahre oder so%w...",
        "Un %ppoisson%w ! Je l'�change contre %rquelque chose de sp�cial%w. Pas de retour ! Je re�ois de nouveaux "
        "articles sp�ciaux %btous les 7 ans environ%w...");
    msg.AutoFormat();
    msg.LoadIntoFont();
    *ev->loadFromMessageTable = false;
...
void BuildEnHyMessage_Bug(IEvent* event) {
    ...
    CustomMessage msg =
        CustomMessage("A tiny %gbug%w! I'll trade you %rsomething special%w for it. No returns! I get new "
                      "special inventory %bevery 7 years or so%w...",
                      "Ein kleiner %gK�fer%w! Ich tausche ihn gegen %retwas Besonderes%w. Und nicht feilschen, okay! "
                      "Ich bekomme neue "
                      "besondere Ware %balle 7 Jahre oder so%w...",
                      "Un petit %ginsecte%w ! Je l'�change contre %rquelque chose de sp�cial%w. Pas de retour ! Je "
                      "re�ois de nouveaux "
                      "articles sp�ciaux %btous les 7 ans environ%w...");
    msg.AutoFormat();
    msg.LoadIntoFont();
