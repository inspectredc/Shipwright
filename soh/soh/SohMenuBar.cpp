...
void DrawRemoteControlMenu() {
    if (ImGui::BeginMenu("Network")) {
        ...
        static std::string AnchorName = CVarGetString(CVAR_REMOTE("AnchorName"), "");
        static std::string anchorRoomId = CVarGetString(CVAR_REMOTE("AnchorRoomId"), "");
        bool isFormValid = !isStringEmpty(CVarGetString(CVAR_REMOTE("IP"), "127.0.0.1")) && port > 1024 && port < 65535 && (
            CVarGetInteger(CVAR_REMOTE("Scheme"), GI_SCHEME_SAIL) != GI_SCHEME_ANCHOR ||
            (
                !isStringEmpty(CVarGetString(CVAR_REMOTE("AnchorName"), "")) &&
                !isStringEmpty(CVarGetString(CVAR_REMOTE("AnchorRoomId"), ""))
            )
        );
        ...
        if (UIWidgets::EnhancementCombobox(CVAR_REMOTE("Scheme"), remoteOptions, GI_SCHEME_SAIL)) {
            switch (CVarGetInteger(CVAR_REMOTE("Scheme"), GI_SCHEME_SAIL)) {
                ...
                case GI_SCHEME_ANCHOR:
                    CVarSetString(CVAR_REMOTE("IP"), "anchor.proxysaw.dev");
                    CVarSetInteger(CVAR_REMOTE("Port"), 43385);
        ...
        if (CVarGetInteger(CVAR_REMOTE("Scheme"), GI_SCHEME_SAIL) == GI_SCHEME_ANCHOR) {
            ImGui::Text("Tunic Color & Name");
            static Color_RGBA8 color = CVarGetColor(CVAR_REMOTE("AnchorColor"), { 100, 255, 100, 255 });
            ...
            if (ImGui::ColorEdit3("##gRemote.AnchorColor", (float*)&colorVec, ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoLabel)) {
                ...
                CVarSetColor(CVAR_REMOTE("AnchorColor"), color);
            ...
            if (ImGui::InputText("##gRemote.AnchorName", (char*)AnchorName.c_str(), AnchorName.capacity() + 1)) {
                CVarSetString(CVAR_REMOTE("AnchorName"), AnchorName.c_str());
                Ship::Context::GetInstance()->GetWindow()->GetGui()->SaveConsoleVariablesOnNextTick();
            }
            ImGui::Text("Room ID");
            int flags = 0;
            ...
            if (ImGui::InputText("##gRemote.AnchorRoomId", (char*)anchorRoomId.c_str(), anchorRoomId.capacity() + 1, flags)) {
                CVarSetString(CVAR_REMOTE("AnchorRoomId"), anchorRoomId.c_str());
                Ship::Context::GetInstance()->GetWindow()->GetGui()->SaveConsoleVariablesOnNextTick();
            }
        }
        ImGui::EndDisabled();
        ...
        if (CVarGetInteger(CVAR_REMOTE("Scheme"), GI_SCHEME_SAIL) == GI_SCHEME_ANCHOR) {
            ...
            if (mAnchorPlayerLocationWindow) {
                if (ImGui::Button(GetWindowButtonText("Player Location Window", CVarGetInteger(CVAR_REMOTE("AnchorPlayerLocationWindow"), 0)).c_str(), ImVec2(ImGui::GetContentRegionAvail().x - 20.0f, 0.0f))) {
            ...
            if (mAnchorLogWindow) {
                if (ImGui::Button(GetWindowButtonText("Incoming Item Window", CVarGetInteger(CVAR_REMOTE("AnchorLogWindow"), 0)).c_str(), ImVec2(ImGui::GetContentRegionAvail().x - 20.0f, 0.0f))) {
            ...
            if (GameInteractor::Instance->isRemoteInteractorEnabled) {
                ...
                switch (CVarGetInteger(CVAR_REMOTE("Scheme"), GI_SCHEME_SAIL)) {
                    ...
                    case GI_SCHEME_CROWD_CONTROL:
                        ...
                        break;
                    case GI_SCHEME_ANCHOR:
                        if (CVarGetInteger(CVAR_REMOTE("AnchorLogWindow"), 0) && mAnchorLogWindow) {
                            mAnchorLogWindow->ToggleVisibility();
                        }
                        if (CVarGetInteger(CVAR_REMOTE("AnchorPlayerLocationWindow"), 0) && mAnchorPlayerLocationWindow) {
                            mAnchorPlayerLocationWindow->ToggleVisibility();
                        }
        ...
        if (GameInteractor::Instance->isRemoteInteractorConnected && CVarGetInteger(CVAR_REMOTE("Scheme"), GI_SCHEME_SAIL) == GI_SCHEME_ANCHOR) {
            ...
            ImGui::Text("%s", CVarGetString(CVAR_REMOTE("AnchorName"), ""));
