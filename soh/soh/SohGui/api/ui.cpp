...
//     SohGui::mSohMenu->AddMenuEntry(label, cvar);
// }

extern "C" void C_AddSidebarEntry(const char* label, int column) {
    SohGui::mSohMenu->AddSidebarEntry("Mods", label, column);
}

// extern "C" void C_RemoveMenuEntry(const char* entryName) {
//     SohGui::mSohMenu->RemoveMenuEntry(entryName);
// }

extern "C" void C_RemoveSidebarEntry(const char* sidebarName) {
    SohGui::mSohMenu->RemoveSidebarEntry("Mods", sidebarName);
}
...
extern "C" void C_AddWidget(const char* sidebar, int column, const char* label, C_WidgetConfig* config) {
    ...
    WidgetInfo& widget = SohGui::mSohMenu->AddWidget(path, label, static_cast<WidgetType>(config->type))
